...
## Dynamic project context file (`.logic-loop/CONTEXT.md`) — build brief

**Origin (2026-09-16):** prior-art review showed a per-project freeform
"Instructions" textarea appended to the agent's system prompt at spawn.
That works only when the app owns the spawn. Logic Loop doesn't (invariant
#4, PTY passthrough), and a textarea is strictly worse than the CLAUDE.md
the user already edits. So: **do not build an instructions editor.** Build
the thing only Logic Loop can build — a generated file of *project state*
that every fresh agent session would otherwise re-derive.

### What it is

One file per project, `.logic-loop/CONTEXT.md`, next to the Idea Board's
`board.md` (Phase 18 set the precedent: app-written, git-committed,
markdown, in the project). Regenerated from the DB whenever its inputs
change. The user opts a project in with one explicit click, which appends
one import line to the project's instruction file(s) so the agent reads
CONTEXT.md at session start.

Rules are human-written and stay in CLAUDE.md/AGENTS.md. **State is
generated.** Never mix the two.

### Content (hard cap 2,000 chars; truncate oldest-first per section)

Rendered top to bottom. Every section is a plain SQL read through
`src/lib/repo.ts` — this is a dumb view over tables (invariant #3), no
model call anywhere in this feature.

1. **Header** (fixed text): file name, "generated by Logic Loop, do not
   edit, regenerated on <ISO ts>", and one line stating that quoted agent
   text below is a record, not an instruction.
2. **Standing decisions** — `decisions` rows for this `cwd` with
   `status = 'answered'` and a non-null `user_answer`, newest first, max 8.
   Render as `- Decided <YYYY-MM-DD>: <question, ≤120 chars> → <user_answer, ≤160 chars>`.
   Read via `listDecisions` (filter in TS, or add a narrow repo helper
   `answeredDecisions(cwd, limit)`). Dismissed/open rows are excluded — an
   open fork is not a standing decision.
3. **Landing note** — `latestLandingNote(cwd)` (`notes` table,
   `kind = 'landing'`), body only, ≤400 chars. Human-written.
4. **Now cards** — `readBoard(project_key)` → `parseBoard` → cards with
   `now: true` (max 3 by Phase 20's cap). Render title + `next:` line if
   present. Human-written.
5. **Open blockers** — `listBlockers` for `cwd` with unresolved status,
   max 5, title only.

Omit any section that is empty. If everything is empty, still write the
file with just the header so the import line never dangles.

### Trust boundary (invariant #5 — this is the whole design constraint)

Feeding extracted transcript text back into an agent's startup context is
a prompt-injection loop. Enforce at the renderer, not by convention:

- **Instruction-position text** (anything the agent could read as a rule)
  may only come from human-authored columns: `decisions.user_answer`,
  `notes.body` (landing), `board.md` cards. These were typed by the user.
- **Agent-authored text** (`decisions.question`, `decisions.assumption`,
  blocker titles extracted from transcripts) is rendered only inside a
  markdown blockquote under the section-2/5 headers, with the header's
  "record, not instruction" line above it. Strip newlines and fence
  markers from it before rendering so it cannot break out of the quote.
- Never include `context_json`, transcript excerpts, tool output, or
  anything from `events`.
- Pure renderer function `renderContext(input): string` in a new
  `src/lib/context.ts` with a `context:check` script (matches the
  existing `*:check` pattern in `package.json`) that asserts: cap
  respected, agent text always quoted, fence/newline injection attempts
  in fixtures cannot escape the quote, empty input still yields the
  header.

### Wiring (explicit click, idempotent, byte-reversible)

Same discipline as `hooks_setup`/`hooks_remove`/`hooks_status` in
`src-tauri/src/ingest.rs` for `~/.claude/settings.json`:

- New Rust commands in `board.rs` (or a sibling `context.rs`):
  `write_context(project_key, content)` mirroring `write_board`
  (`spawn_blocking_result`, `create_dir_all`, atomic write via temp file +
  rename so a half-written file is never read by a session starting at
  that instant).
- `context_import_setup(project_key)`, `context_import_remove`,
  `context_import_status`: append/remove/detect exactly one line,
  `@.logic-loop/CONTEXT.md`, at the end of the project's `CLAUDE.md`
  (create the file with only that line if absent). Idempotent: status
  true ⇒ setup is a no-op. Remove strips only that exact line and leaves
  every other byte untouched. Unit-test both directions the way the
  settings.json tests do.
- Codex and OpenCode read `AGENTS.md`, not `CLAUDE.md`. Same three
  commands take a `target: "claude" | "agents"` arg; the Setup checklist
  offers one checkbox per detected agent file. Antigravity's instruction
  file must be verified against a live `agy` before adding a target —
  do not assume.
- UI lives in the existing Setup checklist (`OnboardingModal.tsx`,
  Phase 31) as one row per project: "Share project state with agents"
  with per-file checkboxes and a "what gets written" expander showing the
  current rendered file verbatim. Never auto-enable. Never enable
  globally.

### Regeneration triggers

Call `regenerateContext(cwd)` (debounced ~500 ms) after: `answerOpenDecisions`,
`setDecisionStatus`, landing-note save (both Auto and Manual paths in
`landing.ts`), any `writeBoard`, `addBlocker`/`setBlockerResolved`/
`resolveAllBlockers`. Skip entirely when `context_import_status` is false
for that project — don't write a file nobody imports. Fail open: a write
error logs and emits through the existing `adapterWarnings` strip; it
never blocks the triggering action.

### Invariant check

- #4: a file read at the *next* session start is process configuration,
  same category as the user editing CLAUDE.md by hand. Nothing is written
  to a live PTY; a running session does not re-read imports, and that is
  correct.
- #3: renderer is pure over repo reads; zero extraction calls.
- #5: enforced structurally above.
- Cost: the 2k cap is a hard ceiling, tested. Phase 33.1 just removed a
  40x per-call overhead; this must not reintroduce a slow one.

### Verification via the `InstructionsLoaded` hook (Claude only)

Claude Code emits an `InstructionsLoaded` hook event (per the CLI's
`mods/types/claude-code.d.ts`) with `file_path`, `memory_type`
(`User | Project | Local | Managed`), `load_reason` (`session_start |
nested_traversal | path_glob_match | include | compact`), and parent-file
relationships for includes. Verify the exact payload against the current
CLI before wiring; the type file is the source of truth. Register it in
`hooks_setup` alongside the existing hooks (same idempotent/reversible
write path; it must survive the settings.json round-trip tests) and route
it through `addHookEvent` like every other event — structured protocol,
invariant #1 clean, nothing parsed from output.

What it buys:

- **Proof the import works.** A session whose `InstructionsLoaded` events
  include `<project>/.logic-loop/CONTEXT.md` with `load_reason: include`
  is confirmed to have read it. Show this as a small "context loaded"
  mark on the tab/session, the same way Phase 31's Connected state only
  turns on after a real tethered event — never on elapsed time or file
  existence.
- **Warning when it didn't.** `context_import_status` true for the
  project, a `SessionStart` seen, but no `InstructionsLoaded` for
  CONTEXT.md by the first `UserPromptSubmit` → emit through the existing
  `adapterWarnings` strip: "CONTEXT.md is imported but this session did
  not load it." Catches a broken/moved import line, a `CLAUDE_CODE_CHILD_
  SESSION`-style environment issue, or a CLI change silently dropping
  includes.
- **Cost visibility, cheap.** The events also list every other
  instruction file loaded (root CLAUDE.md, nested ones, `.claude/rules/*`
  matched by `paths:` globs). Since-you-left could count them; a large
  count on a scratch dir is exactly the Phase 33.1 overhead shape.

Codex/OpenCode have no equivalent event; for `AGENTS.md` targets the
feature stays unverified and the UI must say so rather than imply
success.

### Related, not part of this build: where YAML actually helps

CLAUDE.md has no frontmatter semantics — YAML there is plain text and
should not be used as a "map". The pointer mechanism is `@path`. YAML is
meaningful in `.claude/rules/*.md` via `paths:` globs (path-scoped
loading, Claude only). For this repo that suggests splitting the root
CLAUDE.md's landmines by area into `.claude/rules/` with globs (Rust
landmines load only when `src-tauri/**` is touched), leaving the root as
invariants + process rules + `@` pointers. Separate housekeeping task.

### Companion, separate and smaller: "one instruction file"

One-click setup that writes `@AGENTS.md` into `CLAUDE.md` (idempotent,
same helper as above) so Claude, Codex, and OpenCode read a single
human-maintained file. Zero intelligence, pure setup, also lives in the
Setup checklist. Ship it in the same phase or before; it is the thing most
users actually want first.

### Explicitly out of scope

Any in-app editor for CLAUDE.md/AGENTS.md; per-agent templating;
summarizing transcripts into rules; auto-enabling on new projects; writing
anywhere outside `<project>/.logic-loop/` and the single import line.

### Done means

`npm run context:check` green; Rust unit tests for the import line
(setup/remove/status, byte-identical round-trip, pre-existing content
preserved); `tsc --noEmit`, clippy clean; a manual §-entry in
`docs/TESTING.md` covering: answer a decision → file updates within 1 s →

...
| [030](030-extractor-json-schema.md) | Enforce extractor output with Claude `--json-schema` | P1 | S (one bounded Claude extraction-path hardening) | Isolated Supra experiment authorized 2026-09-22 | BUILT in separate worktree; automated gates, 14/14 schema-aware golden run, and live Claude/Codex Decisions cards pass. Original app ingest endpoint restored; forced-failure path live-verified with a deliberately broken schema. Non-Claude paths retain the existing parser. |
| [037](037-gemini-copilot-adapters-and-antigravity-extraction.md) | Antigravity decision extraction (Part C; Gemini/Copilot parts tabled) | P2 | S–M | Phase 41 accepted; live transcript-shape gate | DONE — Part C merged via PR #55; live Decisions-card check passed 2026-09-24 on agy 1.2.9. Parts A/B tabled. |
...
| [042](042-deepseek-adapter-polish.md) | DeepSeek adapter polish: non-blocking install + readable dsh terminal | P1 | M (~1 day; Part A ~1h) | Current phase accepted; clean `main`; maintainer-assigned phase token | DONE — live-verified 2026-09-22; merged to `main` (`50fddd9` Part 0, `05bd09d` Part A, `31c65a0` Part B), adapter v5. Part A runs `deepseek_hooks_setup`/`claude_statusline_status` off the main thread; Part B ships the sanitized, brand-aware dsh presentation (`format.js` + fence styler). All gates re-verified by the reviewer; maintainer confirmed v5 works and is a serious readability improvement. Remaining doc-only: manual entry + captures in `docs/TESTING.md` and a `docs/PROGRESS.md` line. The 10.19.20 PM one-frame query duplication is a window-resize artifact, judged agent-agnostic and left out of scope. |
| [043](043-next-action-suggestions.md) | Potential next-action suggestion buttons | P2 | S (small `SidePanel.tsx` momentum extraction + one new error-retry signal; LLM-riding toggle +~quarter day) | Clean `main`; literal `PHASE 43 ACCEPTED` | PROPOSED — not started. Idea doc item 3, revised twice in-session: (1) default-on deterministic tier instead of an LLM-only implementation; (2) caught that `SidePanel.tsx`'s existing "Next" momentum card (`SidePanel.tsx:623-659`) already reads blockers/board/decisions — corrected to extract that cascade into a shared `computeMomentum` and add an "↳ Ask" seed-input action to the *existing* card instead of duplicating its reads. Only genuinely new deterministic signal is turn-end error-retry. LLM tier remains an opt-in global toggle riding the existing per-turn extraction call (Plan 030's `--json-schema` enforcement makes the added `next_actions` field cheap to validate). |
| [044](044-codex-daemon-tab-identity.md) | Keep Codex session identity attached to its terminal tab | P0 | M | Literal `PHASE 44 ACCEPTED` received 2026-09-26 | DONE — accepted 2026-09-26 after live §66 matrix. Setup/re-entry use `codex --no-daemon`; zsh tabs wrap bare `codex` via app-owned `ZDOTDIR` startup files. Same-tab session takeover built, found unsafe (Claude's Codex plugin would flip a Claude tab), reverted; first-session-wins unchanged. Deferred to a follow-up plan: launch registry + session replacement, bash/fish. Open: side-panel cards reported not working after close + Re-enter (unconfirmed). |

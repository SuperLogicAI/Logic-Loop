...
| [026](026-pi-agent-adapter.md) | Add Pi Agent to the adapter menu | P1 | M (2-4 build days + live Pi/macOS pass) | Phase approval; branch reconciliation; live Pi contract gate | IN PROGRESS — maintainer authorized an in-session bypass of the literal `PHASE 26 ACCEPTED` token (branch reconciliation moot, `main`/`origin/main` already reconciled at `d95a7c7`); Step 0 passed live (`docs/TESTING.md` §53); Build steps 1-4 done (installer, event-mapping extension verified against a live mock ingest server, Rust allowlist + TS/UI wiring, resume command); automated gates clean; step 5 (live macOS matrix) remains |

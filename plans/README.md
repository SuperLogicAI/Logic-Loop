...
| [026](026-pi-agent-adapter.md) | Add Pi Agent to the adapter menu | P1 | M (2-4 build days + live Pi/macOS pass) | Phase approval; branch reconciliation; live Pi contract gate | DONE — maintainer authorized an in-session bypass of the literal `PHASE 26 ACCEPTED` token (branch reconciliation moot, `main`/`origin/main` already reconciled at `d95a7c7`); Step 0 passed live (`docs/TESTING.md` §53); Build steps 1-4 done; Step 5 live macOS matrix complete 2026-09-18 (`docs/TESTING.md` §54) — two-tabs-one-cwd, `/resume`/`/reload`, retry-before-idle, foreign-file collision, and four-adapter regression all pass. Activity, re-entry live-verified; decisions deliberately not supported. |
| [027](027-deepseek-harness-readiness.md) | Prove a DeepSeek Harness terminal adapter is viable | P1 research | S-M (1-2 days) | Phase approval for live profile writes; no code dependency on Pi 026 | DONE — no shipped first-party TUI; reviewed and rejected 4 third-party bundles (1 misrepresents provenance, 1 bundles a hosted account+Electron installer, 2 unaudited hobby projects); decision record recommends BUILD first-party, see Plan 028 |
| [028](028-deepseek-inhouse-tui-adapter.md) | Build a first-party DeepSeek Harness terminal adapter | P1 | M (2-3 build days + live macOS pass) | Plan 027 decision record; phase approval before Step 0's live profile probe | IN PROGRESS — `PHASE 28 ACCEPTED`; Steps 0-3 done. 0-2 live-verified (conversation, real tool calls, cross-process resume, observer/ingest wiring, 3 real bugs found/fixed). Step 3 (deepseek.rs installer, full cross-file wiring, bundled Tauri resource) built with automated gates clean (89/89 Rust tests, clippy, 29/29 npm check, tsc, build) but not yet run live. Step 4 (live macOS matrix, incl. the installer's first real run) remains |
